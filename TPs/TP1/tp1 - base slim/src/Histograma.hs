...
vacio n (l, u) = Histograma l tamanio (replicate (n + 2) 0)
                  where tamanio = (u - l) / fromIntegral n

-- | Agrega un valor al histograma.
agregar :: Float -> Histograma -> Histograma
agregar x (Histograma l tamanio xs) = Histograma l tamanio (actualizarElem i (+1) xs)
                                      where i = max 0 (min (length xs - 1) (1 + floor ((x - l) / tamanio)))
...
casilleros (Histograma l tamanio xs) = zipWith4 Casillero (infinitoNegativo : limites) (limites ++ [infinitoPositivo]) xs porcentaje
                                        where limites = take (length xs - 1) [l, (l + tamanio)..]
                                              porcentaje = map (\x -> if x == 0 then 0 else (fromIntegral x * 100 / total)) xs
